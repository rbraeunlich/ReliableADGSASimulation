 ...
 * 
 * @param <S> the actual type how the name is formed.
 */
public interface GroupNameProtocol extends CDProtocol {
    ...
    /**
     * Compares this group name with another one and returns true is the name is
     * considered to be as similar to the others, so it can be considered the
     * same groupname again.
     * 
     * @param otherName
     * @return
     */
    ...
    /**
     * Forms a group names, saves it itself (replacing the existing one) and
     * returns it. Because we do not do this in a cycle, we have to pass the
     * node itself.
     */
    GroupName createGroupName(Node node);
